...
bool create_auto_start_task_for_this_user(bool runElevated)
{
    ...
    // ------------------------------------------------------
    // Get the OpenMonitor task folder. Creates it if it doesn't exist.
    hr = pService->GetFolder(_bstr_t(L"\\OpenMonitor"), &pTaskFolder);
    if (FAILED(hr))
    {
        // Folder doesn't exist. Get the Root folder and create the OpenMonitor subfolder.
        ITaskFolder* pRootFolder = NULL;
        hr = pService->GetFolder(_bstr_t(L"\\"), &pRootFolder);
        ExitOnFailure(hr, "Cannot get Root Folder pointer: %x", hr);
        hr = pRootFolder->CreateFolder(_bstr_t(L"\\OpenMonitor"), _variant_t(L""), &pTaskFolder);
        if (FAILED(hr))
        {
            pRootFolder->Release();
            ExitOnFailure(hr, "Cannot create OpenMonitor task folder: %x", hr);
        }
    }
    ...
    // Add an Action to the task. This task will execute the path passed to this custom action.
    {
        ...
        // Set the path of the executable to OpenMonitor (passed as CustomActionData).
    ...
    // ------------------------------------------------------
    //  Save the task in the OpenMonitor folder.
    {
...
bool delete_auto_start_task_for_this_user()
{
    ...
    // ------------------------------------------------------
    // Get the OpenMonitor task folder.
    hr = pService->GetFolder(_bstr_t(L"\\OpenMonitor"), &pTaskFolder);
    if (FAILED(hr))
    {
