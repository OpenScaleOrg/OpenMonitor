...
BOOL CGeneralSettingsDlg::OnInitDialog()
{
    ...
    bool shortcut_exist = CCommon::FileExist((start_up_path + L"\\OpenMonitor.lnk").c_str());
    ...
    if (shortcut_exist)
    {
        ...
        DeleteFile((start_up_path + L"\\OpenMonitor.lnk").c_str());
    ...
    if (COpenMonitorDlg::Instance()->IsGetDiskUsageByPdh())
    {
        const auto& disk_names = COpenMonitorDlg::Instance()->GetPdhDiskUsageHelper().GetDiskNames();
