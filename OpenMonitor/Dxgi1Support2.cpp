...
{
    CallFunctionForEachResource<OPENMONITOR_DV_V_ARGS(&Resource::OnSwapChainResizeBegin)>(m_resource_tracker);
    ...
    CallFunctionForEachResource<OPENMONITOR_DV_V_ARGS(&Resource::OnSwapChainResizeEnd)>(m_resource_tracker, m_p_swap_chain1);
}
...
void CDxgiSwapChain1::Resize(std::uint32_t width, std::uint32_t height)
{
    ...
    ThrowIfFailed<CDxgiException>(
        m_p_swap_chain1->ResizeBuffers(
            ...
            0),
        OPENMONITOR_ERROR_STR("Resize swap chain failed."));
        ...
        [](auto pp_factory)
        {
            *pp_factory = nullptr;
            ...
            ThrowIfFailed<CDxgiException>(
                ...
                OPENMONITOR_ERROR_STR("Create dxgi factory2 failed."));
        },
        [](auto pp_factory)
