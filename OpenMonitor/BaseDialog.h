...
// CBaseDialog dialog
// Used to implement remembering dialog size
// and setting the initial window size as the minimum size
...
public:
    ...
    // Copied from CDialogEx, functionally identical (added handling for slider controls and hyperlink controls)
    ...
    void SetMinSize(int cx, int cy);		// Sets the minimum window size; if not set, the initial window size is used as the minimum
    static HWND GetUniqueHandel(LPCTSTR dlg_name);          // Gets the unique handle of the specified window
    static const std::map<CString, HWND>& AllUniqueHandels();   // Gets the handles of all windows
...
private:
    ...
    // Re-applies layout manager parameters, which resets the base size/position of controls to their current state
