...
// OpenMonitorDlg.cpp : 实现文件
//

#include "stdafx.h"
#include "OpenMonitor.h"
#include "OpenMonitorDlg.h"
...
//静态成员初始化
unsigned int COpenMonitorDlg::m_WM_TASKBARCREATED{ ::RegisterWindowMessage(_T("TaskbarCreated")) };  //注册任务栏建立的消息

COpenMonitorDlg::COpenMonitorDlg(CWnd* pParent /*=NULL*/)
    : CDialog(IDD_OPENMONITOR_DIALOG, pParent)
{
    m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
    m_desktop_dc = ::GetDC(NULL);
}

COpenMonitorDlg::~COpenMonitorDlg()
{
    free(m_pIfTable);
...
CTaskBarDlg* COpenMonitorDlg::GetTaskbarWindow() const
{
...
COpenMonitorDlg* COpenMonitorDlg::Instance()
{
    return dynamic_cast<COpenMonitorDlg*>(theApp.m_pMainWnd);
}

void COpenMonitorDlg::DoDataExchange(CDataExchange* pDX)
{
    CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(COpenMonitorDlg, CDialog)
    ON_WM_QUERYDRAGICON()
    ON_WM_TIMER()
    ON_WM_RBUTTONUP()
    ON_WM_LBUTTONDOWN()
    ON_COMMAND(ID_NETWORK_INFO, &COpenMonitorDlg::OnNetworkInfo)
    ON_COMMAND(ID_ALWAYS_ON_TOP, &COpenMonitorDlg::OnAlwaysOnTop)
    ON_WM_INITMENUPOPUP()
    ON_COMMAND(ID_TRANSPARENCY_100, &COpenMonitorDlg::OnTransparency100)
    ON_COMMAND(ID_TRANSPARENCY_80, &COpenMonitorDlg::OnTransparency80)
    ON_COMMAND(ID_TRANSPARENCY_60, &COpenMonitorDlg::OnTransparency60)
    ON_COMMAND(ID_TRANSPARENCY_40, &COpenMonitorDlg::OnTransparency40)
    ON_WM_CLOSE()
    ON_WM_INITMENU()
    ON_COMMAND(ID_LOCK_WINDOW_POS, &COpenMonitorDlg::OnLockWindowPos)
    ON_WM_MOVE()
    ON_MESSAGE(MY_WM_NOTIFYICON, &COpenMonitorDlg::OnNotifyIcon)
    ON_COMMAND(ID_SHOW_NOTIFY_ICON, &COpenMonitorDlg::OnShowNotifyIcon)
    ON_WM_DESTROY()
    ON_COMMAND(ID_SHOW_CPU_MEMORY, &COpenMonitorDlg::OnShowCpuMemory)
    ON_COMMAND(ID_MOUSE_PENETRATE, &COpenMonitorDlg::OnMousePenetrate)
    ON_COMMAND(ID_SHOW_TASK_BAR_WND, &COpenMonitorDlg::OnShowTaskBarWnd)
    ON_COMMAND(ID_APP_ABOUT, &COpenMonitorDlg::OnAppAbout)
    ON_COMMAND(ID_SHOW_CPU_MEMORY2, &COpenMonitorDlg::OnShowCpuMemory2)
    ON_COMMAND(ID_SHOW_MAIN_WND, &COpenMonitorDlg::OnShowMainWnd)
    ON_COMMAND(ID_CHANGE_SKIN, &COpenMonitorDlg::OnChangeSkin)
    ON_REGISTERED_MESSAGE(m_WM_TASKBARCREATED, &COpenMonitorDlg::OnTaskBarCreated)
    ON_COMMAND(ID_TRAFFIC_HISTORY, &COpenMonitorDlg::OnTrafficHistory)
    ON_WM_MOUSEMOVE()
    ON_WM_LBUTTONDBLCLK()
    ON_COMMAND(ID_OPTIONS, &COpenMonitorDlg::OnOptions)
    ON_COMMAND(ID_OPTIONS2, &COpenMonitorDlg::OnOptions2)
    ON_MESSAGE(WM_EXITMENULOOP, &COpenMonitorDlg::OnExitmenuloop)
    ON_COMMAND(ID_CHANGE_NOTIFY_ICON, &COpenMonitorDlg::OnChangeNotifyIcon)
    ON_COMMAND(ID_ALOW_OUT_OF_BORDER, &COpenMonitorDlg::OnAlowOutOfBorder)
    ON_COMMAND(ID_CHECK_UPDATE, &COpenMonitorDlg::OnCheckUpdate)
    ON_MESSAGE(WM_TASKBAR_MENU_POPED_UP, &COpenMonitorDlg::OnTaskbarMenuPopedUp)
    ON_COMMAND(ID_SHOW_NET_SPEED, &COpenMonitorDlg::OnShowNetSpeed)
    ON_WM_QUERYENDSESSION()
    ON_WM_PAINT()
    ON_MESSAGE(WM_DPICHANGED, &COpenMonitorDlg::OnDpichanged)
    ON_MESSAGE(WM_TASKBAR_WND_CLOSED, &COpenMonitorDlg::OnTaskbarWndClosed)
    ON_MESSAGE(WM_MONITOR_INFO_UPDATED, &COpenMonitorDlg::OnMonitorInfoUpdated)
    ON_MESSAGE(WM_DISPLAYCHANGE, &COpenMonitorDlg::OnDisplaychange)
    ON_WM_EXITSIZEMOVE()
    ON_COMMAND(ID_PLUGIN_MANAGE, &COpenMonitorDlg::OnPluginManage)
    ON_MESSAGE(WM_REOPEN_TASKBAR_WND, &COpenMonitorDlg::OnReopenTaksbarWnd)
    ON_COMMAND(ID_OPEN_TASK_MANAGER, &COpenMonitorDlg::OnOpenTaskManager)
    ON_MESSAGE(WM_SETTINGS_APPLIED, &COpenMonitorDlg::OnSettingsApplied)
    ON_COMMAND(ID_DISPLAY_SETTINGS, &COpenMonitorDlg::OnDisplaySettings)
    ON_WM_LBUTTONUP()
    ON_COMMAND(ID_REFRESH_CONNECTION_LIST, &COpenMonitorDlg::OnRefreshConnectionList)
    ON_MESSAGE(WM_TABLET_QUERYSYSTEMGESTURESTATUS, &COpenMonitorDlg::OnTabletQuerysystemgesturestatus)
    ON_COMMAND(ID_PLUGIN_OPTIONS, &COpenMonitorDlg::OnPluginOptions)
    ON_COMMAND(ID_PLUGIN_DETAIL, &COpenMonitorDlg::OnPluginDetail)
    ON_COMMAND(ID_PLUGIN_OPTIONS_TASKBAR, &COpenMonitorDlg::OnPluginOptionsTaksbar)
    ON_COMMAND(ID_PLUGIN_DETAIL_TASKBAR, &COpenMonitorDlg::OnPluginDetailTaksbar)
    ON_WM_POWERBROADCAST()
    ON_WM_DWMCOLORIZATIONCOLORCHANGED()
END_MESSAGE_MAP()


CString COpenMonitorDlg::GetMouseTipsInfo()
{
    CString tip_info;
    CString temp;
...
void COpenMonitorDlg::SetTransparency()
{
    SetTransparency(theApp.m_cfg_data.m_transparency);
}

void COpenMonitorDlg::SetTransparency(int transparency)
{
...
void COpenMonitorDlg::SetAlwaysOnTop()
{
...
void COpenMonitorDlg::SetMousePenetrate()
{
...
POINT COpenMonitorDlg::CalculateWindowMoveOffset(CRect rect, bool screen_changed)
{
    POINT mov{};    // 所需偏移量
...
void COpenMonitorDlg::CheckWindowPos(bool screen_changed)
{
...
void COpenMonitorDlg::GetScreenSize()
{
...
void COpenMonitorDlg::AutoSelect()
{
...
void COpenMonitorDlg::IniConnection()
{
...
MIB_IFROW COpenMonitorDlg::GetConnectIfTable(int connection_index)
{
...
NetWorkConection COpenMonitorDlg::GetConnection(int connection_index)
{
    if (connection_index >= 0 && connection_index < static_cast<int>(m_connections.size()))
...
void COpenMonitorDlg::IniConnectionMenu(CMenu* pMenu)
{
...
void COpenMonitorDlg::IniTaskBarConnectionMenu()
{
    //向“选择网络连接”子菜单项添加项目
...
void COpenMonitorDlg::SetConnectionMenuState(CMenu* pMenu)
{
...
void COpenMonitorDlg::CloseTaskBarWnd()
{
    if (m_tBarDlg != nullptr)
    {
        if (IsTaskbarWndValid())
...
void COpenMonitorDlg::OpenTaskBarWnd()
{
...
void COpenMonitorDlg::AddNotifyIcon()
{
...
void COpenMonitorDlg::DeleteNotifyIcon()
{
    if (theApp.m_cfg_data.m_show_task_bar_wnd)
        CloseTaskBarWnd();
    //删除通知栏图标
...
void COpenMonitorDlg::ShowNotifyTip(const wchar_t* title, const wchar_t* message)
{
...
void COpenMonitorDlg::UpdateNotifyIconTip()
{
    CString strTip;         //鼠标指向图标时显示的提示
#ifdef _DEBUG
    strTip = CCommon::LoadText(IDS_OPENMONITOR, _T(" (Debug)"));
#else
    strTip = CCommon::LoadText(IDS_OPENMONITOR);
#endif
...
void COpenMonitorDlg::SaveHistoryTraffic()
{
    m_history_traffic.Save();
}

void COpenMonitorDlg::LoadHistoryTraffic()
{
...
void COpenMonitorDlg::BackupHistoryTrafficFile()
{
    CHistoryTrafficFile backup_file(theApp.m_history_traffic_path + L".bak");
...
void COpenMonitorDlg::_OnOptions(int tab, CWnd* pParent)
{
...
void COpenMonitorDlg::ApplySettings(COptionsDlg& optionsDlg)
{
...
void COpenMonitorDlg::SetItemPosition()
{
...
bool COpenMonitorDlg::LoadSkinLayout()
{
...
void COpenMonitorDlg::LoadBackGroundImage()
{
    CImage img_tmp;
    CSize image_size;
    if (theApp.m_cfg_data.m_show_more_info)
...
void COpenMonitorDlg::SetTextFont()
{
    m_skin.SetSettingData(theApp.m_main_wnd_data.ToSkinSettingData());
}

bool COpenMonitorDlg::IsTaskbarWndValid() const
{
    return m_tBarDlg != nullptr && ::IsWindow(m_tBarDlg->GetSafeHwnd());
}

void COpenMonitorDlg::TaskbarShowHideItem(DisplayItem type)
{
    if (IsTaskbarWndValid())
    {
...
void COpenMonitorDlg::CheckClickedItem(CPoint point)
{
    const CSkinFile::Layout& skin_layout{ theApp.m_cfg_data.m_show_more_info ? m_skin.GetLayoutInfo().layout_l : m_skin.GetLayoutInfo().layout_s }; //当前的皮肤布局
...
void COpenMonitorDlg::ApplySkin(int skin_index)
{
...
bool COpenMonitorDlg::IsTemperatureNeeded() const
{
...
// COpenMonitorDlg 消息处理程序

BOOL COpenMonitorDlg::OnInitDialog()
{
...
#ifdef _DEBUG
    atip = CCommon::LoadText(IDS_OPENMONITOR, _T(" (Debug)"));
#else
    atip = CCommon::LoadText(IDS_OPENMONITOR);
#endif
...
//显示。
HCURSOR COpenMonitorDlg::OnQueryDragIcon()
{
...
void COpenMonitorDlg::DoMonitorAcquisition()
{
    //获取网络连接速度
    int rtn{};
    auto getLfTable = [&]() {
...
UINT COpenMonitorDlg::MonitorThreadCallback(LPVOID dwUser)
{
    COpenMonitorDlg* pThis = (COpenMonitorDlg*)dwUser;
    while (true)
    {
        //获取一次监控数据
...
void COpenMonitorDlg::ExitMonitorThread()
{
    // 通知线程退出
...
void COpenMonitorDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    if (nIDEvent == MAIN_TIMER)
    {
        ...
        auto checkNotifyTip = [&](GeneralSettingData::NotifyTipSettings setting_data, int value, int& last_value, int& notify_time, LPCTSTR tip_str)
        {
            if (setting_data.enable)
            {
                ...
                {
                    ShowNotifyTip(CCommon::LoadText(_T("OpenMonitor "), IDS_NOTIFY), tip_str);
        ...
        if (theApp.m_general_data.traffic_tip_enable)
        {
            ...
            if (last_today_traffic < traffic_tip_value && today_traffic >= traffic_tip_value)
            {
                ...
                ShowNotifyTip(CCommon::LoadText(_T("OpenMonitor "), IDS_NOTIFY), info.GetString());
            }
...
void COpenMonitorDlg::OnRButtonUp(UINT nFlags, CPoint point)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
...
void COpenMonitorDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
    CheckClickedItem(point);
...
void COpenMonitorDlg::OnNetworkInfo()
{
    // TODO: 在此添加命令处理程序代码
    //弹出“连接详情”对话框
...
void COpenMonitorDlg::OnAlwaysOnTop()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnTransparency100()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnTransparency80()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnTransparency60()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnTransparency40()
{
    // TODO: 在此添加命令处理程序代码
...
BOOL COpenMonitorDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        CTest::TestCommand();
        //ShowNotifyTip(CCommon::LoadText(_T("OpenMonitor "), IDS_NOTIFY), _T("测试通知"));
...
void COpenMonitorDlg::OnInitMenu(CMenu* pMenu)
{
...
BOOL COpenMonitorDlg::PreTranslateMessage(MSG* pMsg)
{
    // TODO: 在此添加专用代码和/或调用基类
    //屏蔽按回车键和ESC键退出
...
void COpenMonitorDlg::OnLockWindowPos()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnMove(int x, int y)
{
    CDialog::OnMove(x, y);
...
afx_msg LRESULT COpenMonitorDlg::OnNotifyIcon(WPARAM wParam, LPARAM lParam)
{
...
void COpenMonitorDlg::OnShowNotifyIcon()
{
    // TODO: 在此添加命令处理程序代码
    if (theApp.m_general_data.show_notify_icon)
    {
...
void COpenMonitorDlg::OnDestroy()
{
    CDialog::OnDestroy();
...
void COpenMonitorDlg::OnShowCpuMemory()
{
    // TODO: 在此添加命令处理程序代码
    CRect rect;
    GetWindowRect(rect);
...
void COpenMonitorDlg::OnShowCpuMemory2()
{
    // TODO: 在此添加命令处理程序代码
    if (IsTaskbarWndValid())
    {
...
void COpenMonitorDlg::OnMousePenetrate()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnShowTaskBarWnd()
{
    // TODO: 在此添加命令处理程序代码
    if (m_tBarDlg != nullptr)
    {
...
void COpenMonitorDlg::OnAppAbout()
{
    // TODO: 在此添加命令处理程序代码
    //弹出“关于”对话框
    CAboutDlg aDlg;
...
//当资源管理器重启时会触发此消息
LRESULT COpenMonitorDlg::OnTaskBarCreated(WPARAM wParam, LPARAM lParam)
{
    if (m_tBarDlg != nullptr)
    {
        CloseTaskBarWnd();
...
void COpenMonitorDlg::OnShowMainWnd()
{
    // TODO: 在此添加命令处理程序代码
    if (!theApp.m_cfg_data.m_hide_main_window)
    {
...
void COpenMonitorDlg::OnChangeSkin()
{
    // TODO: 在此添加命令处理程序代码
    CSkinDlg skinDlg;
    //初始化CSkinDlg对象的数据
...
void COpenMonitorDlg::OnTrafficHistory()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnMouseMove(UINT nFlags, CPoint point)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
...
void COpenMonitorDlg::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
    CheckClickedItem(point);
    if (m_clicked_item.IsPlugin() && m_clicked_item.PluginItem() != nullptr)
...
void COpenMonitorDlg::OnOptions()
{
    _OnOptions(0, this);
}
...
void COpenMonitorDlg::OnOptions2()
{
    CWnd* pParent = this;
    if (IsTaskbarWndValid())
        pParent = m_tBarDlg;
...
afx_msg LRESULT COpenMonitorDlg::OnExitmenuloop(WPARAM wParam, LPARAM lParam)
{
...
void COpenMonitorDlg::OnChangeNotifyIcon()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnAlowOutOfBorder()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnCheckUpdate()
{
    // TODO: 在此添加命令处理程序代码
...
afx_msg LRESULT COpenMonitorDlg::OnTaskbarMenuPopedUp(WPARAM wParam, LPARAM lParam)
{
    //设置“选择连接”子菜单项中各单选项的选择状态
...
//任务栏窗口切换显示网速时的处理
void COpenMonitorDlg::OnShowNetSpeed()
{
    // TODO: 在此添加命令处理程序代码
    if (IsTaskbarWndValid())
    {
...
BOOL COpenMonitorDlg::OnQueryEndSession()
{
    ...
    if (theApp.m_debug_log)
    {
        CCommon::WriteLog(_T("OpenMonitor进程已被终止，设置已保存。"), (theApp.m_config_dir + L".\\debug.log").c_str());
    }
...
afx_msg LRESULT COpenMonitorDlg::OnDpichanged(WPARAM wParam, LPARAM lParam)
{
    static int dpi;
    static COpenMonitorDlg* pThis;
    dpi = LOWORD(wParam);
    pThis = this;
...
afx_msg LRESULT COpenMonitorDlg::OnTaskbarWndClosed(WPARAM wParam, LPARAM lParam)
{
    theApp.m_cfg_data.m_show_task_bar_wnd = false;
    //关闭任务栏窗口后，如果没有显示通知区图标，且没有显示主窗口或设置了鼠标穿透，则将通知区图标显示出来
    if (!theApp.m_general_data.show_notify_icon && theApp.IsForceShowNotifyIcon())
...
afx_msg LRESULT COpenMonitorDlg::OnMonitorInfoUpdated(WPARAM wParam, LPARAM lParam)
{
...
LRESULT COpenMonitorDlg::OnDisplaychange(WPARAM wParam, LPARAM lParam)
{
    GetScreenSize();
    CheckWindowPos(true);
    return 0;
...
void COpenMonitorDlg::OnExitSizeMove()
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
    CheckWindowPos();
...
void COpenMonitorDlg::OnPluginManage()
{
    // TODO: 在此添加命令处理程序代码
...
LRESULT COpenMonitorDlg::OnReopenTaksbarWnd(WPARAM wParam, LPARAM lParam)
{
    CloseTaskBarWnd();
    OpenTaskBarWnd();
    return 0;
...
void COpenMonitorDlg::OnOpenTaskManager()
{
    ShellExecuteW(NULL, _T("open"), (theApp.m_system_dir + L"\\Taskmgr.exe").c_str(), NULL, NULL, SW_NORMAL);       //打开任务管理器
}


afx_msg LRESULT COpenMonitorDlg::OnSettingsApplied(WPARAM wParam, LPARAM lParam)
{
...
void COpenMonitorDlg::OnDisplaySettings()
{
    // TODO: 在此添加命令处理程序代码
...
void COpenMonitorDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
    //CheckClickedItem(point);
    //if (m_clicked_item.IsPlugin() && m_clicked_item.PluginItem() != nullptr)
...
void COpenMonitorDlg::OnRefreshConnectionList()
{
    IniConnection();
...
afx_msg LRESULT COpenMonitorDlg::OnTabletQuerysystemgesturestatus(WPARAM wParam, LPARAM lParam)
{
    return 0;
}


void COpenMonitorDlg::OnPluginOptions()
{
...
void COpenMonitorDlg::OnPluginDetail()
{
    if (m_clicked_item.IsPlugin())
    {
        //找到对应的插件
...
void COpenMonitorDlg::OnPluginOptionsTaksbar()
{
...
void COpenMonitorDlg::OnPluginDetailTaksbar()
{
    if (IsTaskbarWndValid() && m_tBarDlg->GetClickedItem().IsPlugin())
    {
        //找到对应的插件
...
UINT COpenMonitorDlg::OnPowerBroadcast(UINT nPowerEvent, LPARAM nEventData)
{
    // 系统从休眠恢复
    if (nPowerEvent == PBT_APMRESUMESUSPEND)
    {
        ...
        static COpenMonitorDlg* pThis = this;
...
void COpenMonitorDlg::OnColorizationColorChanged(DWORD dwColorizationColor, BOOL bOpacity)
{
