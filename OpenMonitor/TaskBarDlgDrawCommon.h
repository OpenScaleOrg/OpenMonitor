...
// 0.0039215686 = 1 / 255
#define OPENMONITOR_ONE_IN_255_NUM 0.0039215686
// 0.0039215686 + 0.0000784314
#define OPENMONITOR_ONE_IN_255_NUM_CEIL 0.004
#define OPENMONITOR_ONE_IN_255_NUM_FLOOR 0.0038431372
#define OPENMONITOR_ONE_IN_255_CEIL OPENMONITOR_STR(OPENMONITOR_ONE_IN_255_NUM_CEIL)
#define OPENMONITOR_ONE_IN_255_FLOOR OPENMONITOR_STR(OPENMONITOR_ONE_IN_255_NUM_FLOOR)
...
#define OPENMONITOR_MAKE_TAG_TYPE(tag_name) \
    struct tag_name                            \
    {                                          \
    };

        OPENMONITOR_MAKE_TAG_TYPE(WhenTupleAtFirst)
        OPENMONITOR_MAKE_TAG_TYPE(WhenTupleAtLast)
        OPENMONITOR_MAKE_TAG_TYPE(WhenTupleAtMiddle)
...
#define OPENMONITOR_CD2D1BITMAPCACHE_LOCK_CACHE_MAP_AND_EXPIRE_INTERVAL(sp_data) \
    std::lock_guard<std::mutex> cache_and_expire_interval_lock_guard(sp_data->m_mutex)

#define OPENMONITOR_CD2D1BITMAPCACHE_LOCK_GC_INTERVAL(sp_data) \
    ...
    std::thread m_gc_thread{
        [wp_data = std::weak_ptr<HeapData>(m_sp_data)]()
        {
            do
            {
                ...
                {
                    OPENMONITOR_CD2D1BITMAPCACHE_LOCK_GC_INTERVAL(sp_data);
