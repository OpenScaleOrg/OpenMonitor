...
void CD3D10Device1::Recreate(Microsoft::WRL::ComPtr<IDXGIAdapter1> p_adapter1)
{
    ...
    ThrowIfFailed(
        ...
        OPENMONITOR_ERROR_STR("Create D3D10Device1 failed."));
        ...
        result.Construct([](DeviceVector* p_content)
            ...
            do
            {
                ...
                else if (FAILED(hr))
                {
                    ThrowIfFailed(
                        hr,
                    OPENMONITOR_ERROR_STR("EnumAdapters1 failed. Maybe ppAdapter parameter is NULL."));
                }
            } while (true); });
    }
    ...
    if (m_is_config_changed)
    {
        if (!CDllFunctions::D3DCompile.HasValue())
        {
            throw std::runtime_error{
                OPENMONITOR_ERROR_STR("Can not find function D3DCompile in d3dcompiler_47.dll or d3dcompiler_47.dll is not exist.")};
        }
        ...
        ThrowIfFailed<CDXShaderException>(
            CDllFunctions::D3DCompile(
                ...
                &p_error_message),
            OPENMONITOR_ERROR_STR("Compile DX shader failed."),
            p_error_message);
...
CD3D10DrawCallWaiter::CD3D10DrawCallWaiter(Microsoft::WRL::ComPtr<ID3D10Device1> p_device)
{
    ...
    ThrowIfFailed<CD3D10Exception1>(
        ...
        OPENMONITOR_ERROR_STR("Create ID3D10Query failed."));
    m_p_query->End();
}
