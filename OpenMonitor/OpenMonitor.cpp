...
BEGIN_MESSAGE_MAP(COpenMonitorApp, CWinApp)
    //ON_COMMAND(ID_HELP, &CWinApp::OnHelp)
    ON_COMMAND(ID_HELP, &COpenMonitorApp::OnHelp)
    ON_COMMAND(ID_FREQUENTY_ASKED_QUESTIONS, &COpenMonitorApp::OnFrequentyAskedQuestions)
    ON_COMMAND(ID_UPDATE_LOG, &COpenMonitorApp::OnUpdateLog)
END_MESSAGE_MAP()


COpenMonitorApp* COpenMonitorApp::self = NULL;


// COpenMonitorApp 构造
COpenMonitorApp::COpenMonitorApp()
{
    self = this;
    // 支持重新启动管理器
...
void COpenMonitorApp::LoadLanguageConfig()
{
    CIniHelper ini{ m_config_path };
    m_general_data.language.fromConfigString(ini.GetString(_T("general"), _T("language"), L""));

    // On first run (no language saved), default to English for non-Chinese systems
    // instead of falling through to the built-in Simplified Chinese resource.
    if (m_general_data.language.bcp_47.empty())
    {
        wchar_t locale_name[LOCALE_NAME_MAX_LENGTH]{};
        if (GetUserDefaultLocaleName(locale_name, LOCALE_NAME_MAX_LENGTH) > 0)
        {
            wstring locale(locale_name);
            if (locale.find(L"zh") == wstring::npos)
            {
                m_general_data.language.bcp_47 = L"en-US";
                m_general_data.language.display_name = L"English";
                m_general_data.language.translator = L"";
            }
        }
    }
}

void COpenMonitorApp::LoadConfig()
{
    CSettingsHelper ini;
...
void COpenMonitorApp::SaveConfig()
{
    CSettingsHelper ini;
...
void COpenMonitorApp::LoadPluginDisabledSettings()
{
    CIniHelper ini{ m_config_path };
    m_cfg_data.plugin_disabled.FromString(ini.GetString(L"config", L"plugin_disabled", L""));
}

void COpenMonitorApp::LoadGlobalConfig()
{
    bool portable_mode_default{ false };
    wstring global_cfg_path{ m_module_dir + L"global_cfg.ini" };
    if (!CCommon::FileExist(global_cfg_path.c_str()))       //如果global_cfg.ini不存在，则根据AppData/Roaming/OpenMonitor目录下是否存在config.ini来判断配置文件的保存位置
    {
...
void COpenMonitorApp::SaveGlobalConfig()
{
...
int COpenMonitorApp::DPI(int pixel)
{
    return m_dpi * pixel / 96;
}

void COpenMonitorApp::DPI(CRect& rect)
{
...
void COpenMonitorApp::DPIFromWindow(CWnd* pWnd)
{
    CWindowDC dc(pWnd);
...
void COpenMonitorApp::CheckUpdate(bool message)
{
    ...
    {
        CString info;
        if (contents_en.empty())
            info.Format(CCommon::LoadText(IDS_UPDATE_AVLIABLE), version.c_str());
        else
            info.Format(CCommon::LoadText(IDS_UPDATE_AVLIABLE2), version.c_str(), contents_en.c_str());

        if (AfxMessageBox(info, MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
            wstring appinstaller_link = update_helper.GetLinkAppInstaller();
            if (!appinstaller_link.empty())
            {
                // Open the .appinstaller URL — Windows App Installer handles
                // download, verification and in-place update automatically.
                ShellExecute(NULL, _T("open"), appinstaller_link.c_str(), NULL, NULL, SW_SHOW);
            }
            else
            {
                // Fallback: open the ZIP download page in the browser
                ShellExecute(NULL, _T("open"), link.c_str(), NULL, NULL, SW_SHOW);
            }
        }
    }
    else
...
void COpenMonitorApp::CheckUpdateInThread(bool message)
{
    AfxBeginThread(CheckUpdateThreadFunc, (LPVOID)message);
}

UINT COpenMonitorApp::CheckUpdateThreadFunc(LPVOID lpParam)
{
...
UINT COpenMonitorApp::InitOpenHardwareMonitorLibThreadFunc(LPVOID lpParam)
{
...
bool  COpenMonitorApp::SetAutoRun(bool auto_run, bool task_scheduler)
{
    if (task_scheduler)
    {
        //使用任务计划的方式设置开机自启动
...
bool COpenMonitorApp::GetAutoRun(wstring* auto_run_path, bool task_scheduler)
{
    ...
    else
    {
        ...
        wchar_t buff[256];
        ULONG size{ 256 };
        if (key.QueryStringValue(APP_NAME, buff, &size) == ERROR_SUCCESS)       //如果找到了“OpenMonitor”键
        {
            if (auto_run_path != nullptr)
            {
                //保存路径
            ...
            return (m_module_path_reg == buff); //如果“OpenMonitor”的值是当前程序的路径，就返回true，否则返回false
        }
        else
        {
            return false;       //没有找到“OpenMonitor”键，返回false
        }
    }
}

bool COpenMonitorApp::SetAutoRunByRegistry(bool auto_run)
{
    CRegKey key;
    //打开注册表项
    if (key.Open(HKEY_CURRENT_USER, _T("Software\\Microsoft\\Windows\\CurrentVersion\\Run")) != ERROR_SUCCESS)
...
bool COpenMonitorApp::SetAutoRunByTaskScheduler(bool auto_run)
{
...
CString COpenMonitorApp::GetSystemInfoString()
{
    CString info;
    info += _T("System Info:\r\n");
...
void COpenMonitorApp::InitMenuResourse()
{
    //载入菜单
...
HICON COpenMonitorApp::GetMenuIcon(UINT id)
{
...
void COpenMonitorApp::AutoSelectNotifyIcon()
{
...
// 唯一的一个 COpenMonitorApp 对象

COpenMonitorApp theApp;


// COpenMonitorApp 初始化

BOOL COpenMonitorApp::InitInstance()
{
    //替换掉对话框程序的默认类名
    WNDCLASS wc;
    ...
    LPCTSTR mutex_name{};
#ifdef _DEBUG
    mutex_name = _T("OpenMonitor-e8Ahk24HP6JC8hDy");
#else
    mutex_name = _T("OpenMonitor-1419J3XLKL1w8OZc");
#endif
...
void COpenMonitorApp::InitOpenHardwareLibInThread()
{
#ifndef WITHOUT_TEMPERATURE
...
void COpenMonitorApp::UpdateOpenHardwareMonitorEnableState()
{
#ifndef WITHOUT_TEMPERATURE
...
//void COpenMonitorApp::UpdateTaskbarWndMenu()
//{
//    //获取“显示设置”子菜单
...
bool COpenMonitorApp::IsForceShowNotifyIcon()
{
...
std::wstring COpenMonitorApp::GetPlauginTooltipInfo() const
{
...
bool COpenMonitorApp::IsTaksbarItemDisplayed(CommonDisplayItem item) const
{
    if (item.IsPlugin())
    {
...
void COpenMonitorApp::SendSettingsToPlugin()
{
...
void COpenMonitorApp::UpdatePluginMenu(CMenu* pMenu, ITMPlugin* plugin, int plugin_cmd_start_index)
{
    if (pMenu != nullptr)
    {
        //删除菜单已经存在的插件命令
...
void COpenMonitorApp::CheckWindows11Taskbar()
{
...
bool COpenMonitorApp::DPIFromRect(const RECT& rect, UINT* out_dpi_x, UINT* out_dpi_y)
{
...
unsigned int COpenMonitorApp::GetThemeColor() const
{
    return m_theme_color;
}

void COpenMonitorApp::SetThemeColor(COLORREF color)
{
    m_theme_color = color;
}

void COpenMonitorApp::OnHelp()
{
    // TODO: 在此添加命令处理程序代码
    ShellExecute(NULL, _T("open"), _T("https://github.com/OpenScaleOrg/OpenMonitor/wiki"), NULL, NULL, SW_SHOW);
}


void COpenMonitorApp::OnFrequentyAskedQuestions()
{
    ShellExecute(NULL, _T("open"), _T("https://github.com/OpenScaleOrg/OpenMonitor/blob/master/Help.md"), NULL, NULL, SW_SHOW);
}


void COpenMonitorApp::OnUpdateLog()
{
    ShellExecute(NULL, _T("open"), _T("https://github.com/OpenScaleOrg/OpenMonitor/blob/master/UpdateLog/update_log.md"), NULL, NULL, SW_SHOW);
}


int COpenMonitorApp::ExitInstance()
{
    // 释放GDI+
...
int COpenMonitorApp::GetAPIVersion()
{
    return 0;
}

const wchar_t* COpenMonitorApp::GetVersion()
{
    return VERSION;
}

double COpenMonitorApp::GetMonitorValue(MonitorItem item)
{
    switch (item)
    {
...
const wchar_t* COpenMonitorApp::GetMonitorValueString(MonitorItem item, int is_main_window)
{
...
void COpenMonitorApp::ShowNotifyMessage(const wchar_t* strMsg)
{
    COpenMonitorDlg* pMainWnd = dynamic_cast<COpenMonitorDlg*>(m_pMainWnd);
    if (pMainWnd != nullptr)
    {
        pMainWnd->ShowNotifyTip(CCommon::LoadText(IDS_OPENMONITOR_PLUGIN_NITIFICATION), strMsg);
    }
}

unsigned short COpenMonitorApp::GetLanguageId() const
{
    return m_general_data.language.language_id;
}

const wchar_t* COpenMonitorApp::GetPluginConfigDir() const
{
...
int COpenMonitorApp::GetDPI(DPIType type) const
{
    COpenMonitorDlg* pMainWnd = dynamic_cast<COpenMonitorDlg*>(m_pMainWnd);
    switch (type)
    {
...
const wchar_t* COpenMonitorApp::GetStringRes(const wchar_t* key, const wchar_t* section)
{
