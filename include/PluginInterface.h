...
public:
    ...
    */
    ...
    /**
     * @brief   插件初始化
     * @detail  当插件被加载时被调用，传递IOpenMonitor接口的指针。插件可以保存此指针以调用IOpenMonitor接口中的函数
     * @param   pApp
     */
    virtual void OnInitialize(IOpenMonitor* pApp) {}
};
...
public:
    /**
     * @brief   获取此接口的版本。
     * @attention 插件在调用IOpenMonitor中的函数时需要先判断接口的版本
     * @return  int
     */
    virtual int GetAPIVersion() = 0;

    /**
     * @brief   获取OpenMonitor的版本。
     * @return  const wchar_t*
     */
